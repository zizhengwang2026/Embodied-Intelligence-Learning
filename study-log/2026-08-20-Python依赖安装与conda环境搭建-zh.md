...
- 装环境这件事，等真做 DEA 研究时会变成“第一道坎”：软体/具身的开源代码动不动依赖 MuJoCo、有限元库、特定 numpy 版本，**版本一乱全崩**。conda 隔离 = 给每个实验一台“干净小机器”。
- 养成习惯：**一个项目一个环境、requirements 写清楚**——这能让别人复现你的 DEA 实验，也能让你三个月后还跑得动自己的代码。今天学的就是研究基建。
