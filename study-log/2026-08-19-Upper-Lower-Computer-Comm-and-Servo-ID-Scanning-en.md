...
- A servo is like a badged employee: the bus **polls it by ID**, ask-and-answer returns an angle. A DEA has no encoder and no factory ID — to know whether it moved and how much, you must add external **flexible sensing (capacitive / strain)** and bring readings back over the same upper/lower-computer channel.
- So “reading state” in soft robotics isn't handed to you by a servo protocol — it's the **sensing problem** your direction must solve (Day 17 soft modeling meets it again). The architecture carries over; the sensor has to change.
