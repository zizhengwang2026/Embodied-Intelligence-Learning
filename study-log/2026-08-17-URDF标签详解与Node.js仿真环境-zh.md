...
- （revolute / prismatic **必需**；continuous / fixed 不需要）`<limit lower="..." upper="..." effort="..." velocity="..."/>`：限制转角范围 / 力矩 / 速度。
...
<mark>**📌 课件补充｜装 Node.js 选 LTS 还是 Current**：官网 nodejs.org 下载时有两条线——**LTS（长期支持版）**：更稳定、维护周期长，推荐给绝大部分用户；**Current（最新版）**：含最新功能但可能不稳，适合尝鲜。**企业/学习开发请务必选 LTS**。安装时保持默认选项即可，安装器会自动把 `node`、`npm` 命令加进系统环境变量，无需手动配置。另外，Node.js 不只是"跑仿真"——它还能写后端服务器/API、跑前端框架（React/Vue）的构建工具、写命令行工具，还能免费使用 npm 上百万个开源包。</mark>
...
| 7 | "一个 link 可以有两个 parent（比如并联机构：末端同时经两条链连回基座）。" | 标准 URDF 是**树**，一个 link 只能有一个 parent。注意"两条腿共用躯干"其实是**合法**的（一个 parent 可以有多个 child）；真正做不了的是**闭环**——并联机构需用 SDF / MJCF 等更灵活的格式或把环中一个关节设为 fixed。 |

---

## 6. DEA 交叉链接（轻量，非主线）

- 这一讲把机器人“写”成 XML：每个 link 是刚体零件、每个 joint 是确定的旋转/平移轴。标准 URDF 天生只描述**刚体**——DEA 人工肌肉这种“一通电整片变形”的软体驱动，既没有固定的 joint 轴，也没有不变形的 link，硬塞进去就失真。
- 想在仿真里试 DEA，得换**软体仿真**思路（很多小刚体拼成链 / 有限元），或用“身体即算法”的**形态计算**视角：让变形本身替控制分担一部分活（Day 3 已埋过这个点）。Day 17 软体建模会把这些工具补上。
