...
```mermaid
sequenceDiagram
    participant R as 机器人 / 舵机
    participant S as WebSocket 服务端
    participant W as 网页前端
    R->>S: 上报角度 JSON {id, angle}
    S-->>W: 实时推送（WebSocket）
    W->>W: 渲染实时角度 / 曲线
    W->>S: 控制指令（可选）
    S->>R: 下发指令
...
- 今天搭的“角度→网页实时刷新”管道，本质是**给执行器装实时仪表盘**。DEA 实验同样需要：把电压、电容传感读数、估算应变实时画出来，你才看得见迟滞和漂移。
- WebSocket/AI 编程这套“要什么说什么、小段验证”的打法，特别适合软体驱动这种**定制化强、现成库少**的调试场景——自己写高压驱动和传感采集的监控界面，正是 vibe coding 的用武之地。
