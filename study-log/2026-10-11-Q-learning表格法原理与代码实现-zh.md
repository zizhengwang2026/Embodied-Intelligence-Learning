...
### 2.4 ε-贪婪探索与离策略（off-policy）
- **探索 ε-贪婪**：以 ε 概率随机选动作去探索未知状态，1−ε 概率贪心选 max Q 利用已知——前期大 ε 多探索，后期减小利用。
- **离策略 off-policy**：Q-learning 的更新目标用的是「下一态最大 Q」对应的贪心动作，不要求当前这步真按它走；这点和“边走边学”的 on-policy 方法不同。
